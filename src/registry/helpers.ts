import { posix as path, dirname } from "node:path";
...
export function getModulePatterns(rootDir: string): Array<string> {
    // Convert root to posix once
  const posixRoot = rootDir.replace(/\\/g, "/");

  return [
    path.join(posixRoot, "tools", "**", "*.js"),
    path.join(posixRoot, "resources", "**", "*.js"),
    path.join(posixRoot, "prompts", "**", "*.js"),
  ];
}
...
export function getRootDir(importMetaUrl: string): string {
  ...
  const buildRoot = dirname(registryDir);
  return buildRoot;
}
