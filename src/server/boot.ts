...
export async function boot(
  ...
  app.use((req, res, next) => {
    logger.info(`Incoming Request`, { method: req.method, url: req.url, ip: req.ip, headers: req.headers, body: req.body });
    next();
  });
  ...
  app.use((err: Error, req: express.Request, res: express.Response, next: express.NextFunction) => {
    logger.error(`Unhandled Error: ${err.message}`, { stack: err.stack, method: req.method, url: req.url, ip: req.ip });
    if (!res.headersSent) {
  ...
  const httpServer = app.listen(port, () => {
    logger.info(`Jotty MCP Server (HTTP) listening on http://localhost:${String(port)}/mcp`);
